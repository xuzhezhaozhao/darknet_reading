...
/*
void train_lsd3(char *fcfg, char *fweight, char *gcfg, char *gweight, char
*acfg, char *aweight, int clear)
{
#ifdef GPU
...
printf("%d: gen: %f, adv: %f | gen_avg: %f, adv_avg: %f, %f rate, %lf seconds,
%d images\n", i, floss, aloss, floss_avg, aloss_avg, get_current_rate(gnet),
...
/*
void train_pix2pix(char *cfg, char *weight, char *acfg, char *aweight, int
clear)
{
#ifdef GPU
  ...
  while (get_current_batch(gnet) < maxbatch) {
    ...
    for (j = 0; j < gnet->subdivisions; ++j) {
      ...
      *gnet->seen += gnet->batch;
      ...
      // printf("realness %f\n", cuda_mag_array(imerror,
      // imlayer.outputs*imlayer.batch));
      // printf("features %f\n",
      // cuda_mag_array(gnet->layers[gnet->n-1].delta_gpu,
      // imlayer.outputs*imlayer.batch));
      ...
      /*
         ...
         printf("%d: %f %f\n", k, mean_array(l.output, l.outputs*l.batch),
...
void train_colorizer(char *cfg, char *weight, char *acfg, char *aweight,
  ...
  while (get_current_batch(net) < net->max_batches) {
    ...
    printf(
        "%d: gen: %f, adv: %f | gen_avg: %f, adv_avg: %f, %f rate, %lf "
        "seconds, %d images\n",
        i, gloss, aloss, gloss_avg, aloss_avg, get_current_rate(net),
        sec(clock() - time), i * imgs);
    if (i % 1000 == 0) {
...
/*
   void train_lsd2(char *cfgfile, char *weightfile, char *acfgfile, char
*aweightfile, int clear)
   {
#ifdef GPU
char *train_images = "/home/pjreddie/data/coco/trainvalno5k.txt";
char *backup_directory = "/home/pjreddie/backup/";
...
while (get_current_batch(net) < net->max_batches) {
    ...
    for(j = 0; j < net->subdivisions; ++j){
        ...
        *net->seen += net->batch;
        ...
        printf("features %f\n", cuda_mag_array(imlayer.delta_gpu,
imlayer.outputs));
        printf("realness %f\n", cuda_mag_array(imerror, imlayer.outputs));
...
void run_lsd(int argc, char **argv) {
  ...
  // else if(0==strcmp(argv[2], "train2")) train_lsd2(cfg, weights, acfg,
  // aweights, clear);
  // else if(0==strcmp(argv[2], "traincolor")) train_colorizer(cfg, weights,
  // acfg, aweights, clear);
  // else if(0==strcmp(argv[2], "train3")) train_lsd3(argv[3], argv[4], argv[5],
  // argv[6], argv[7], argv[8], clear);
