...
move pick_move(mcts_tree *tree, float temp, int player) {
  ...
  */
  ...
  fprintf(stderr,
          "%d %d, Result: %f, Prior: %f, Prob: %f, Mean Value: %f, Child "
          "Result: %f, Visited: %d\n",
...
/*
   float predict_move(network *net, float *board, float *move, int multi, float
   *ko, float temp)
   {
   ...
   fprintf(stderr, "%d %d, Result: %f, Prior: %f, Prob: %f, Mean Value: %f,
   Child Result: %f, Visited: %d\n", max_i/19, max_i%19, root->result,
   root->prior[max_i], root->prob[max_i], root->mean[max_i],
   (root->children[max_i])?root->children[max_i]->result:0,
   root->visit_count[max_i]);
   fprintf(stderr, "%d %d, Result: %f, Prior: %f, Prob: %f, Mean Value: %f,
   Child Result: %f, Visited: %d\n", ind/19, ind%19, root->result,
   root->prior[ind], root->prob[ind], root->mean[ind],
   (root->children[ind])?root->children[ind]->result:0, root->visit_count[ind]);
...
/*
   move generate_move(mcts_tree *root, network *net, int player, float *board,
int multi, float temp, float *ko, int print)
   {
   move m = {0};
//root = run_mcts(tree, network *net, float *board, float *ko, int n, float
cpuct)
int i, j;
int empty = 1;
for(i = 0; i < 19*19; ++i){
  ...
  while (1) {
    ...
    if (!strcmp(buff, "protocol_version")) {
      ...
      if (anon) {
        ...
        printf(
            "=%s 1.0. Want more DarkGo? You can find me on OGS, unlimited "
            "games, no waiting! https://online-go.com/user/view/434218\n\n",
            ids);
      }
      ...
      scanf("%s", comm);
      int known =
          (!strcmp(comm, "protocol_version") || !strcmp(comm, "name") ||
           !strcmp(comm, "version") || !strcmp(comm, "known_command") ||
           !strcmp(comm, "list_commands") || !strcmp(comm, "quit") ||
           !strcmp(comm, "boardsize") || !strcmp(comm, "clear_board") ||
           !strcmp(comm, "komi") || !strcmp(comm, "final_status_list") ||
           !strcmp(comm, "play") || !strcmp(comm, "genmove_white") ||
           !strcmp(comm, "genmove_black") || !strcmp(comm, "fixed_handicap") ||
           !strcmp(comm, "genmove"));
      if (known)
        printf("=%s true\n\n", ids);
      else
