...
float_pair get_rnn_vid_data(network net, char **files, int n, int batch,
  ...
  for (b = 0; b < batch; ++b) {
    ...
    for (i = 0; i < net.batch; ++i) {
      memcpy(feats + (b + i * batch) * output_size, output + i * output_size,
...
void train_vid_rnn(char *cfgfile, char *weightfile) {
  ...
  while (get_current_batch(net) < net.max_batches) {
    ...
    fprintf(stderr, "%d: %f, %f avg, %f rate, %lf seconds\n", i, loss, avg_loss,
            get_current_rate(net), sec(clock() - time));
    if (i % 100 == 0) {
      char buff[256];
...
void generate_vid_rnn(char *cfgfile, char *weightfile) {
  ...
  int i;
  CvCapture *cap = cvCaptureFromFile(
      "/extra/vid/ILSVRC2015/Data/VID/snippets/val/"
      "ILSVRC2015_val_00007030.mp4");
  float *feat;
  float *next;
  image last;
  for (i = 0; i < 25; ++i) {
