...
void validate_classifier_crop(char *datacfg, char *filename, char *weightfile) {
  ...
  for (i = 1; i <= splits; ++i) {
    ...
    printf("%d: top 1: %f, top %d: %f, %lf seconds, %d images\n", i,
           avg_acc / i, topk, avg_topk / i, sec(clock() - time), val.X.rows);
    free_data(val);
  }
}
...
void try_classifier(char *datacfg, char *cfgfile, char *weightfile,
  ...
  while (1) {
    ...
    image im = crop_image(r, (r.w - 224 - 1) / 2 + 1, (r.h - 224 - 1) / 2 + 1,
                          224, 224);
...
void predict_classifier(char *datacfg, char *cfgfile, char *weightfile,
  ...
  while (1) {
    ...
    for (i = 0; i < top; ++i) {
      ...
      // names[index], predictions[index], (net->hierarchy->parent[index] >= 0)
      // ? names[net->hierarchy->parent[index]] : "Root");
      // else printf("%s: %f\n",names[index], predictions[index]);
...
void threat_classifier(char *datacfg, char *cfgfile, char *weightfile,
  ...
  while (1) {
    ...
    draw_box_width(out, x2 + border, y1 + .02 * h, x2 + .5 * w,
                   y1 + .02 * h + border, border, 0, 0, 0);
    if (threat > .97) {
      draw_box_width(out, x2 + .5 * w + border, y1 + .02 * h - 2 * border,
                     x2 + .5 * w + 6 * border, y1 + .02 * h + 3 * border,
                     3 * border, 1, 0, 0);
    }
    draw_box_width(out, x2 + .5 * w + border, y1 + .02 * h - 2 * border,
                   x2 + .5 * w + 6 * border, y1 + .02 * h + 3 * border,
                   .5 * border, 0, 0, 0);
    draw_box_width(out, x2 + border, y1 + .42 * h, x2 + .5 * w,
                   y1 + .42 * h + border, border, 0, 0, 0);
    if (threat > .57) {
      draw_box_width(out, x2 + .5 * w + border, y1 + .42 * h - 2 * border,
                     x2 + .5 * w + 6 * border, y1 + .42 * h + 3 * border,
                     3 * border, 1, 1, 0);
    }
    draw_box_width(out, x2 + .5 * w + border, y1 + .42 * h - 2 * border,
                   x2 + .5 * w + 6 * border, y1 + .42 * h + 3 * border,
    ...
    for (i = 0; i < threat * h; ++i) {
      ...
      draw_box_width(out, x1 + border, y2 - border - i, x2 - border,
                     y2 - border - i, 1, r, g, 0);
    }
    top_predictions(net, top, indexes);
    char buff[256];
