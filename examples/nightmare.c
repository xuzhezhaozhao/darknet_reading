...
void run_lsd(int argc, char **argv)
{
    srand(0);
    if(argc < 3){
        fprintf(stderr, "usage: %s %s [cfg] [weights] [image] [options!
(optional)]\n", argv[0], argv[1]);
        return;
    }
...
rate, thresh, norm);
        ...
        char buff[256];
        if (prefix){
            sprintf(buff, "%s/%s_%s_%d_%06d",prefix, imbase, cfgbase, max_layer,
e);
        }else{
        ...
        image crop = crop_image(im, im.w * (1. - zoom)/2., im.h * (1.-zoom)/2.,
im.w*zoom, im.h*zoom);
...
void run_nightmare(int argc, char **argv) {
  ...
  for (e = 0; e < rounds; ++e) {
    ...
    for (n = 0; n < iters; ++n) {
      ...
      fflush(stderr);
      if (reconstruct) {
        reconstruct_picture(net, features, im, update, rate, momentum, lambda,
                            smooth_size, 1);
        // if ((n+1)%30 == 0) rate *= .5;
        ...
        int layer = max_layer + rand() % range - range / 2;
        int octave = rand() % octaves;
        optimize_picture(net, im, layer, 1 / pow(1.33333333, octave), rate,
                         thresh, norm);
      }
    }
    fprintf(stderr, "done\n");
