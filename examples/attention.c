 ...
 */
    ...
    printf(
        "%ld, %.3f: Att: %f, %f avg, Class: %f, %f avg, %f rate, %lf seconds, "
        "%ld images\n",
        get_current_batch(net), (float)(*net->seen) / N, aloss, avg_att_loss,
        closs, avg_cls_loss, get_current_rate(net),
...
void validate_attention_single(char *datacfg, char *filename,
                               char *weightfile) {
  int i, j;
