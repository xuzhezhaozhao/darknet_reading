    ...
    for (i = 0; i < 90; ++i) {
      ...
      for (k = 0; k < l.w * l.h; ++k) {
        ...
        if (v) {
          ...
          axpy_cpu(ids, 1, l.output + b * l.outputs + l.classes * l.w * l.h + k,
                   l.w * l.h, l.sums[i], 1);
          ++l.counts[i];
        }
      }
...
void forward_iseg_layer_gpu(const layer l, network net) {
  ...
  for (b = 0; b < l.batch; ++b) {
    activate_array_gpu(l.output_gpu + b * l.outputs, l.classes * l.w * l.h,
                       LOGISTIC);
    // if(l.extra) activate_array_gpu(l.output_gpu + b*l.outputs +
    // l.classes*l.w*l.h, l.extra*l.w*l.h, LOGISTIC);
  }
...
void backward_iseg_layer_gpu(const layer l, network net) {
  int b;
  for (b = 0; b < l.batch; ++b) {
    // if(l.extra) gradient_array_gpu(l.output_gpu + b*l.outputs +
